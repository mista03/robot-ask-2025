...
void setup() {
  ...
  while (distance > stopDist) {
    float coeff = clamp((float)distance / (compareDist - stopDist), 0, 1);
    Serial.print("coef = ");
    Serial.println(coeff);

    uint8_t nextSpeed = coeff * (motorSpeed - lowestSpeed) + lowestSpeed;
    changeSpeed(currentSpeed, nextSpeed, 10);
